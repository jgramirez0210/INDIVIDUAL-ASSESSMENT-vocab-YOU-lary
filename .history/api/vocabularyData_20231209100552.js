...
const getSingleWord = (firebaseKey) => new Promise((resolve, reject) => {
  fetch(`https://vocab-you-lary-f729-default-rtdb.firebaseio.com/vocabulary/books/${firebaseKey}.json`, {
    method: 'GET',    headers: {      'Content-Type': 'applicatio/json',
    },  }) .then((rspose) => response.jsn())
 .then((dta)=> {   if (daa)      resove(ta);
   } else{     reject(ne Error(`No word found with firebaseKy: ${irebaseKey}`));
   } }) .cach(rejct);
});const filteryHtml = () => new Promise((resolve, reject) => {
