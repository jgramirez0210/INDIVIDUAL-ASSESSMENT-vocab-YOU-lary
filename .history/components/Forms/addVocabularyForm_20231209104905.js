...
const addVocabForm = (obj = {}) => {
  ...
  const domString = `
    <form id="${obj.firebaseKey ? `update-vocabulary--${obj.firebaseKey}` : 'submit-vocabulary'}" class="mb-4">
      ...
      <input type="text" class="form-control" id="vocabulary-language" aria-described by="vocabulary-language" placeholder="Enter a Vocabulary Language" value="${obj.language || ''}" required>
      </div>  
</select>
      </div>
